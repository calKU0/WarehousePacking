...
namespace WarehousePacking.Infrastructure.Data
{
    public class DapperDbExecutor : IDbExecutor
    {
        ...
        public async Task<IEnumerable<T>> QueryAsync<T>(string sql, object? param = null, CommandType? commandType = CommandType.StoredProcedure, Connection connectionName = Connection.ERPConnection)
        {
        ...
        public async Task<TFirst?> QuerySingleOrDefaultAsync<TFirst, TSecond, TThird>(string sql, Func<TFirst, TSecond, TThird, TFirst> map, string splitOn, object? param = null, CommandType? commandType = CommandType.StoredProcedure, Connection connectionName = Connection.ERPConnection)
        {
            var connectionString = _config.GetConnectionString(connectionName.ToString())
                ?? throw new InvalidOperationException($"Connection string '{connectionName}' not found.");
        ...
        public async Task<T?> QuerySingleOrDefaultAsync<T>(string sql, object? param = null, CommandType? commandType = CommandType.StoredProcedure, Connection connectionName = Connection.ERPConnection)
        {
            var connectionString = _config.GetConnectionString(connectionName.ToString())
                ?? throw new InvalidOperationException($"Connection string '{connectionName}' not found.");
